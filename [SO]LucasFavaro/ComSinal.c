...
void teste(int rdnum)
{
	...
	if ((estado[rdnum] > VAZIO && sinal == 0 && ((rdnum == 0)||rdnum == 2))||(estado[rdnum] > VAZIO && sinal == 1 && ((rdnum == 1)||rdnum == 3))) {
		//checa o sinal para saber se pode avançar
		sleep(0);
	//	printf ("Carro vindo da pista %d está passando no cruzamento, ninguém a direita\n", rdnum+1); // para debugar
		entrou[rdnum]=1;
		contador++;
	}
}
...
void* controle(){
	//controla o sinal, quando abre ou fecha (0 ou 1)
	int gerador;
	int i;
	srand(time(0));
	while(1){
